...
- `--no-expand-repeat-escape`: リピート脱出を展開せず、従来どおりコメントとして残します。
- `--global-loop-count N`: グローバルループ点以降を `N` 回ぶん展開します。既定は `2` です。
...
- **リピート脱出/全体ループ**: MewMMLPad に `[ ... / ... ]` 相当の構文がないため、脱出を含むループは展開されます。MDX のグローバルループ点以降も `--global-loop-count` 回ぶん展開します。チャンネルごとのループ区間長が違う場合は、最長のループ区間に合わせて短いチャンネルへ休符を追加します。
